...
bool MemoryManager::init(int max_slots, size_t input_tensor_bytes, size_t output_tensor_bytes) {
    ...
    try {
        ...
        for (int i = 0; i < max_slots; ++i) {
            ...
            slot->metas_.clear();
            slot->assigned_stream_ids_.clear();
            slot->stream_ = nullptr;
            slot->state_.store(Slot::State::Empty);
            ...
            if (err_in != cudaSuccess || err_out != cudaSuccess || err_ev != cudaSuccess) {
...
bool MemoryManager::adjustSlotsForMode(int grid_mode) {
    ...
    int streams = 1;
    if (grid_mode == 1) streams = 1;
    else if (grid_mode == 2) streams = 2;
    else if (grid_mode == 3) streams = 4;

    int desired = base_slots_ * streams;
    int slot_limit = getSlotLimit();
